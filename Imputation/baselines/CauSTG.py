    ...
    def __init__(self):
        ...
        for i in range(self.seasonal_number):
            ...
            self.seasonal_layer.append(nn.Conv2d(in_channels=1,
                                ...
                                padding=(0,padding)))
        self.trend_layer = nn.Linear(24,24)   
...
class CauSTG(BaseModel):
    def __init__(self, dropout=0.3, supports=None, gcn_bool=True, addaptadj=True, in_dim=2,out_dim=12,nhid=32,kernel_size=2,blocks=4,layers=2):
    ...
    def forward(self, input, known_set, transform):
        in_len = input.size(3)
        ...
        for i in range(self.blocks * self.layers):
            ...
            if self.gcn_bool and self.supports is not None:
                # if self.addaptadj:
                #     temp_sups = []
                #     for supp in new_supports:
                #         t_supp = supp[:, known_set]
                #         t_supp = t_supp[known_set, :]

                #         temp_sups.append(t_supp)
                #     x = self.gconv[i](x, temp_sups)
                # else:
                temp_sups = []
