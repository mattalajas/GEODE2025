...
class LSJSTN(BaseModel):
    ...
    def forward(self, x, predefined_A, transform):
        """
...
class TAttn(nn.Module):
    ...
    def __init__(self, dim_in, dim_out=None):
        ...
        if dim_out is None:
            dim_out = dim_in * 4
        self.w_query = nn.Parameter(torch.empty(dim_in, dim_out))
        self.w_key   = nn.Parameter(torch.empty(dim_in, dim_out))
        self.bias    = nn.Parameter(torch.zeros(dim_out))
        self.trans   = nn.Parameter(torch.empty(dim_out, 1))

        init.xavier_uniform_(self.w_query)
        init.xavier_uniform_(self.w_key)
        init.xavier_uniform_(self.trans)
