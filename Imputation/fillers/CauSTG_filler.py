...
class Filler(pl.LightningModule):
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        x = F.pad(x,(1,0,0,0))
        y = rearrange(y, 'b t n d -> b d n t')
        eval_mask = rearrange(eval_mask, 'b t n d -> b d n t')

        batch_data["input"] = x  # b s n2 d
        batch_data["known_set"] = list(range(mask.shape[2]))

        # Compute outputs and rescale
        res = self.predict_batch(batch, preprocess=False, postprocess=True)
        imputation = res[0]
        imputation = imputation.transpose(1, 3)
...
class CauSTGFiller(Filler):
    def __init__(self,
        ...
        adj_mx = load_adj(adj, adjtype)
        supports = [torch.tensor(i).to(device) for i in adj_mx]

        model_kwargs['supports'] = supports

        super(CauSTGFiller, self).__init__(model_class=model_class,
    ...
    def training_step(self, batch, batch_idx):
        ...
        imputation, pred_trend, pred_season = res[0], res[1], res[2]
        imputation = imputation.transpose(1, 3)
        ...
        with torch.no_grad():
            real_trend, real_season = self.model.decomposed_sea_tre(y)
        
        loss_sea = torch_metrics.mape(pred_season,real_season,eval_mask)
        ...
        eval_mask = rearrange(eval_mask, 'b t n d -> b d n t')
        loss_main = self.loss_fn(imputation, target, eval_mask)
        loss = loss_main + self.y1*loss_sea + self.y2*loss_tre + self.y3*loss_reg
