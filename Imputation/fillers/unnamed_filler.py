...
class unKrigFiller(pl.LightningModule):
    def __init__(self,
                 model_class,
                 model_kwargs,
                 optim_class,
        ...
        """
        super(unKrigFiller, self).__init__()
...
class UnnamedKrigFiller(unKrigFiller):
    def __init__(self,
                 model_class,
                 model_kwargs,
                 optim_class,
    ...
    #         env_loss = torch.cat(
    #                     [env_loss,
    #                      self.loss_fn(fin_irm_all_s[i], target, mask.bool()).unsqueeze(0)])
    #         # env_loss = torch.cat(
    #         #             [env_loss,
    ...
    #         # MMD for inv 
    #         mmds = torch.cat([mmds, mmd_loss(reco[0], reco[2]).unsqueeze(0)])
    #         mmds = torch.cat([mmds, mmd_loss(reco[1], reco[3]).unsqueeze(0)])
    #     recon_loss = mmds.mean()

    #     main_loss = self.loss_fn(finpreds, fin_target, stack_mask.bool()) 
    ...
    def training_step(self, batch, batch_idx):
        ...
        # opt2.zero_grad()
        # # Edge Conv loss
        # loss_edge = self.y3 * torch.sqrt(self.loss_fn_2(pred_scores[0], pred_scores[1], pred_scores[2].bool()))
        # self.manual_backward(loss_edge, retain_graph=True)
        # opt2.step()
        ...
        main_loss = self.loss_fn_1(finpreds, target, stack_mask.bool()) 
        loss = main_loss + self.y1 * recon_loss + self.y2 * irm_loss
