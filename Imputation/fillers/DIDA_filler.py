...
class Filler(pl.LightningModule):
    ...
    def predict_batch(self, batch, preprocess=False, postprocess=True, return_target=False):
        ...
        # Rescale outputs
        if postprocess:
            css, sss = y_hat[0], y_hat[1]
            cs = self._postprocess(css, batch_preprocessing)
            ss = self._postprocess(sss, batch_preprocessing)
            y_hat = [cs, ss]
...
class DidaFiller(Filler):
    def __init__(self,
                 ...
                 y1=1,
                 horizon=24):
        super(DidaFiller, self).__init__(model_class=model_class,
    ...
    def training_step(self, batch, batch_idx):
        ...
        env_loss = torch.tensor([]).to(device=mask.device)
        for _ in range(self.int_times):
            s1 = np.random.randint(len(ss))
            s = torch.sigmoid(ss[s1]).detach()
            conf = s * cs
            # conf=self.model.comb_pred(cs,)
            env_loss = torch.cat(
                [env_loss, self.loss_fn(conf, target, mask).unsqueeze(0)])
        env_mean = env_loss.mean()
        env_var = torch.var(env_loss * self.int_times)
        penalty = env_mean + env_var

        cs_loss = self.loss_fn(cs, target, mask)

        loss = cs_loss + self.y1 * penalty
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Compute predictions and compute loss
        imputation, _ = self.predict_batch(batch, preprocess=False, postprocess=False)
