...
class Filler(pl.LightningModule):
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        y = y[:, :, :split]
        mask = mask[:, :, :split]
        eval_mask = eval_mask[:, :, :split]

        adj = self.adj[:split, :split]
        c_adj = self.adj[split:, :split]
        c_e_index, c_e_weight = dense_to_sparse(torch.tensor(c_adj).to(x.device))

        x_temp = rearrange(x, 'b t n d -> (b t) n d')
        poi_x = self.simp_conv(x_temp, c_e_index)
        poi_x = rearrange(poi_x, '(b t) n d -> b t n d', b=x.size(0), t=x.size(1))
        poi_x = poi_x[:, :, :split]
        x = x[:, :, :split]
        ...
        poi_x = rearrange(poi_x, 'b t n d -> n (b t) d')
        mean, std = torch.mean(poi_x), torch.std(poi_x)
        poi_x = (poi_x - mean) / (std + 1e-5)

        poi_x_arr = poi_x.to('cpu').detach().numpy()
        poi_adj = np.zeros_like(adj)
        for i in range(poi_adj.shape[0]):
            for j in range(poi_adj.shape[0]):
                poi_adj[i, j] = pearsonr(poi_x_arr[i], poi_x_arr[j])[0]
        fs, neigh_fs = self.heterogeneous_relations(poi_adj, full, self.K)
        fs = torch.tensor(fs).to(x.device)
        neigh_fs = torch.tensor(neigh_fs).to(x.device)

        neigh_fs_flat = neigh_fs.view(-1)
        neigh_fs_feat = x[neigh_fs_flat]
        neigh_fs_feat = neigh_fs_feat.view(n, self.K, b*s, d)

        x_fs = torch.permute(neigh_fs_feat, (0, 2, 1, 3))
        
        x_fs = x_fs[full]  # b s n1 d, n1 = num of observed entries
        fin_fs = fs[full].unsqueeze(1).unsqueeze(1)
        ...
        batch_data["x_fs"] = x_fs
        batch_data["fs"] = fin_fs
        batch_data["TE"] = TE
        batch_data["T"] = self.horizon

        # Compute outputs and rescale
        imputation = self.predict_batch(batch, split, preprocess=False, postprocess=True)
    ...
    def predict_loader(self, loader, preprocess=False, postprocess=True, return_mask=True):
        ...
        for batch in loader:
            ...
            y_hat = self.predict_batch(batch, split, preprocess=preprocess, postprocess=postprocess)
...
class INCREASE_FS_Filler(Filler):
    def __init__(self,
                 model_class,
                 model_kwargs,
                 optim_class,
                 ...
                 horizon=24,
                 num_n=5):
        super(INCREASE_FS_Filler, self).__init__(model_class=model_class,
        ...
        self.simp_conv = SimpleConv(aggr='mean')
    ...
    def training_step(self, batch, batch_idx):
        ...
        b, s, n, d = mask.size()
        cur_entry_num = n
        mask_sum = rearrange(mask, "b s n 1 -> (b s) n").sum(0)  # n
        ...
        imputation = self.predict_batch(batch, split, preprocess=False, postprocess=False)

        if self.scaled_target:
            target = batch.transform['y'].transform(y)
        else:
            target = y
            imputation = self._postprocess(imputation, split, batch_preprocessing)
