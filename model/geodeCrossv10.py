...
class GeodeNBCD(nn.Module):
    ...
    def forward(self, x_fwd, edge_index, edge_weight=None):
        ...
        return output_invars, output_vars, res_att

class GeodeCrossV10(BaseModel):
    def __init__(self,
                 input_size,
                 hidden_size,
                 output_size,
                 ...
                 att_heads=8,
                 nbcd_layers=1):
        super(GeodeCrossV10, self).__init__()
        ...
        self.temp_att = TransformerLayer(input_size=hidden_size,
                                         hidden_size=hidden_size,
                                         ff_size=hidden_size,
                                         n_heads=att_heads,
                                         axis='time',
    ...
    def forward(self,
        ...
        for layer in self.nbcds:
            x_fwd, output_vars, res_att = layer(x_fwd, edge_index) 
        ...
        # Cross GCN
        x_zeros = torch.zeros_like(xh_inv_3).to(device=device)
        cr_embs = torch.cat((x_zeros, tr_embs), dim=2)

        c_adj = c_adj[:, tr_indx]
        cr_edge_index, cr_edge_weight = dense_to_sparse(c_adj.T)
        cr_edge_index[0] += x_fwd.shape[2]
        
        for layer in self.gcn_cross:
            cr_caus, cross_att = layer(cr_embs, cr_edge_index, cr_edge_weight, need_weights=True)
            cr_caus = self.activation(cr_caus)
            cr_embs = self.layernorm1(cr_caus + cr_embs)
        cross_embs = cr_embs[:, :, :xh_inv_3.shape[2]]
        ...
        traf_nodes = cross_embs[:, :, ar_indx]
        traf_nodes = rearrange(traf_nodes, 'b t n d -> b (t n) d')
        traf_nodes = F.normalize(traf_nodes, dim=-1, eps=EPSILON)

        samp_mask = torch.diag(torch.ones(t*n_air)).to(device)
        samp_mask = samp_mask.unsqueeze(0).repeat(b, 1, 1)
